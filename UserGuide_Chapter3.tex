...
different radius, which create an approximately D-shaped profile. Because of the finite
...
\begin{description}
\item{\texttt{secondary\_cycle }:} This switch controls how the coolant pumping power in the first wall and blanket is determined, and also how the calculation of the plant's thermal to electric conversion efficiency (the secondary cycle thermal efficiency) proceeds. See also Section~\ref{sec:powerflow}, which describes the flow of power through the fusion power core.

If \texttt{secondary\_cycle = 0}, the efficiency of the power generation cycle is set to a single value obtained from previous cycle modelling studies.  The heat deposited in the divertor coolant is assumed to be at such low temperature that it cannot be used for power generation and is dumped to the environment.  The mechanical power consumed by the primary coolant pumps is estimated as a fixed fraction of the heat transported.
...
In the remaining options (\texttt{secondary\_cycle = 1, 2 or 3}), a more detailed routine~\cite{Harrington_bop} takes user inputs for the coolant outlet temperature (which may be used as an iteration variable), the internal coolant channel diameter, and the segmentation of the blanket and calculates the maximum temperature in the armour. If this temperature is above user-specified material limits, the outer diameter of the first wall pipes is reduced, up to a limit determined by the internal coolant pressure. The mechanical pumping power required for the first wall and breeder zone is then calculated from thermal-hydraulic formulae, assuming these are separate coolant loops with the same inlet and outlet temperatures. The isentropic efficiency of the first wall and blanket coolant pumps (enthalpy increase in the fluid for isentropic compression divided by the mechanical power used) is specified by the parameter \texttt{etaiso}.  Note that the mechanical pumping powers for the shield and divertor are still calculated using the simplified method (a fixed fraction of the heat transported).  The heat deposited in the divertor coolant is used for power generation.
...
An entirely new capital cost model is being developed, and will be described in~\cite{kovari_cost}.
...
The net electric power is calculated in routine \texttt{POWER}. It is possible that the net electric power can become negative due to a high recirculating power. Switch
...
Switch \texttt{ireactor} determines whether the cost of electricity is calculated. If \texttt{ireactor = 0}, no cost of electricity calculation is performed. If \texttt{ireactor = 1}, then the cost of electricity is evaluated, with the value quoted in units of \$/MWh.
