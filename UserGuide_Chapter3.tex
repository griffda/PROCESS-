...
If the central solenoid is superconducting, a steel case is assumed to
surround the current-carrying winding pack. Its cross-sectional area is
determined by the $J \times B$ hoop force on the coil divided by the allowable
hoop stress, calculated as the lower of the two quantities (2/3 *
\texttt{csytf}) and (0.5 * \texttt{csutf}), where input parameter
\texttt{csytf} is the yield strength of the case in Pascals, and
\texttt{csutf} is the ultimate strength of the case in Pascals. (This is the
same calculation as for the allowable stress in the TF coil cases.)
...
beginning-of-pulse (`BOP') and at the EOF. To limit the current density at the
BOP, constraint equation no.\ 27 should be turned on with iteration variable
no.\ 39 (\texttt{fjohc0}). To limit the current density at the EOF, constraint
