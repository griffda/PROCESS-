...
switch \texttt{iradloss}. In each case, the quantity \texttt{pscaling} is
derived directly from the energy confinement scaling (see above;
\texttt{pscaling} is $P_{\mbox{\scriptsize loss}}$ in
Equation~\ref{eqn:ploss}).
\begin{description}
...
\caption[Power balance within the core plasma] {\label{fig:powerflow1}
  \textit{Schematic diagram of the flow of power within the plasma core
    (assuming switch \texttt{iradloss=1}). Variable names are given in
    [\ldots]. Refer to Figure~\ref{fig:powerflow2} for the onward flow of
    power from the five red-bordered boxes, while the source of the power in
    the blue box is indicated in Figure~\ref{fig:powerflow3}.} }
\end{figure}
...
Figure~\ref{fig:powerflow1}. (This assumes \texttt{iradloss=1} --- see
Section~\ref{sec:iradloss}; other values for \texttt{iradloss} are equivalent
to subtle differences in the positioning of the arrows leaving the core plasma
power balance circle in Figure~\ref{fig:powerflow1}.)
...
(In reality, the blanket comprises the first wall plus the so-called breeding
...
\item{\texttt{blktmodel}:} This switch determines whether a simple blanket
  neutronics model or a more comprehensive treatment based on recent full
  neutronics analyses for EFDA~\cite{efda_blanket_model} is used.
  ...
  (see switch \texttt{blkttype} below). The nuclear heating of the TF coil
  superconductor is calculated assuming an exponential neutron attenuation
  through the blanket and shield materials, based on 1990 ITER data.
  ...
  applied to satisfy the engineering requirements. For further details of this
  model, see Section~\ref{sec:blanket_neutronics}.

  Note that \texttt{blktmodel = 1} forces \texttt{blktcycle = 0} and
  \texttt{blkttype = 3} (see below).

\item{\texttt{blktcycle}:} This switch controls how the coolant pumping power
  in the first wall and blanket is determined, and also how the calculation of
  the plant's thermal to electric conversion efficiency (the secondary cycle
  thermal efficiency) proceeds. See also Section~\ref{sec:powerflow}, which
  describes the flow of power through the fusion power core.

  If \texttt{blktcycle = 0}, a simplified model calculates the respective
  powers deposited in the first wall, breeder zone, shield and divertor, and
  calculates the coolant pumping powers for these systems from a user
  specified fraction of the total heat deposited in the coolant. The thermal
  efficiency of the secondary cycle is determined from the choice of blanket
  type (see \texttt{blkttype} below) and is a fixed value for the each of the
  three options (WCLL, HCLL, HCPB --- see Section~\ref{sec:etath}).

  If \texttt{blktcycle $>$ 0}, a more detailed routine~\cite{Harrington_bop}
  takes user inputs for the coolant outlet temperature (which may be used as
  an iteration variable), the internal coolant channel diameter, and the
  segmentation of the blanket and calculates the maximum temperature in the
  first wall. If the first wall temperature is above user-specified material
  limits, the outer diameter of the first wall pipes is reduced, up to a limit
  determined by the internal coolant pressure. The pumping power for the first
  wall and breeder zone is then calculated from thermal-hydraulic formulae,
  assuming these are separate coolant loops with the same inlet and outlet
  temperatures. Note that the pumping powers for the shield and divertor are
  still calculated using the simplified method. The secondary cycle thermal
  efficiency (\texttt{etath}) is calculated from the coolant outlet
  temperature using simple relations between temperature and efficiency from
  previous cycle modelling studies (see also Section~\ref{sec:etath}):
  \begin{itemize}
  \item If \texttt{blktcycle = 1}, the input value for \texttt{etath} is used.
  \item If \texttt{blktcycle = 2}, a steam Rankine cycle is used to calculate
    \texttt{etath}.
  \item If \texttt{blktcycle = 3}, a supercritical CO$_2$ cycle is used to
    calculate \texttt{etath}.
  \end{itemize}

\item{\texttt{blkttype}:} This switch allows the user to choose between
  different blanket compositions. This determines the breeder materials (and
...
Breeder material         & liquid LiPb-Li   & liquid LiPb-Li   & solid Li$_2$O-Be \\
Coolant fluid            & water            & water            & helium           \\
Coolant switch \texttt{coolwh} &  2         &   2              &  1$^*$           \\ \hline
Li$_2$O fraction         & \texttt{fblli2o} & \texttt{fblli2o} &     ---          \\
beryllium fraction       & \texttt{fblbe}   & \texttt{fblbe}   &     ---          \\
LiPb fraction            &     ---          &     ---          & \texttt{fbllipb} \\
lithium fraction         &     ---          &     ---          & \texttt{fblli}   \\
coolant fraction         & \texttt{vfblkt}  & \texttt{vfblkt}  & \texttt{vfblkt}  \\
stainless steel fraction & \texttt{fblss}   & \texttt{fblss}   & \texttt{fblss}   \\
vanadium fraction        & \texttt{fblvd}   & \texttt{fblvd}   & \texttt{fblvd}   \\ \hline
\end{tabular}
\end{center}
...
{\label{tab:blkttype}
  \textit{Summary of the materials comprising the blanket in the various scenarios
    available assuming \texttt{blktmodel=0}. ( $^*$If \texttt{blktmodel=1} ---
    see Section~\ref{sec:blanket_neutronics} --- \texttt{blkttype=3} is
    assumed, but \texttt{coolwh=2} as the KIT model assumes that, other than
    the first wall/blanket, the rest of the fusion power core is
    water-cooled.) The fractions given are all available to be modified, and 
    should of course add up to 1.0 for any given model.}
}
\end{table}
...
of auxiliary \textit{heating}\/ power (in MW) to be applied to the
...
\Red{(Note that these figures and the following description assume that switch
...
neutron power. The power necessary to drive the divertor coolant pumps,
\texttt{htpmw\_div} in Figure~\ref{fig:powerflow2}, is assumed to be a
fraction $f$ (= input parameter \texttt{fpumpdiv}) of the total thermal power
absorbed by the divertor, excluding the coolant pump power itself:
\begin{equation}
...
Figure~\ref{fig:powerflow3}). (N.B.\ see also the caption for
Table~\ref{tab:etath}.)
...
fraction $(1-e^{d/\lambda})$ is assumed to be absorbed in the first wall
(of thickness $d$), given by the neutron decay length $\lambda$ in the first wall
(input parameter \texttt{declfw}). In addition, the orbit loss fraction of the
neutral beam power (if present), and the alpha power fraction lost directly
from the plasma also contribute to the total thermal power absorbed by the first wall.

If switch \texttt{blktcycle = 0} (see Section~\ref{sec:blanket_switches}), the
first wall coolant pump power is a fraction \texttt{fpumpfw} of the total
(non-pumping) thermal power absorbed by the first wall.

If switch \texttt{blktcycle > 0}, a detailed thermal hydraulic model for the
first wall (and breeding zone) is used~\cite{Harrington_bop}, as outlined in
Section~\ref{sec:blanket_switches}. A number of input parameters may be
used to adjust the model, as follows:
\begin{itemize}
\item \texttt{coolp} is the first wall coolant pressure (Pa);
\item \texttt{afwi, afwo} are the inner radii of the inboard and outboard
  first wall/blanket coolant channels (m);
\item \texttt{inlet\_temp} is the inlet temperature of the coolant for blanket and first wall (K);
\item \texttt{outlet\_temp} is the outlet temperature of coolant for blanket
  and first wall (K) (N.B.\ this is a calculated quantity if water is the coolant);
\item \texttt{nblktmodpi, nblktmodpo} are the number of inboard and outboard
  blanket modules in the poloidal direction;
\item \texttt{nblktmodti, nblktmodto} are the number of inboard and outboard
  blanket modules in the toroidal direction;
\item \texttt{tfwmatmax} is the maximum temperature of the first wall material (K);
\item \texttt{etaiso} is the isentropic efficiency of the first wall and blanket coolant pumps;
\item \texttt{fwerlim} is the erosion thickness allowance for the first wall (m).
\end{itemize}

The efficiency of the first wall coolant pump is \texttt{etahtp}.

\subsubsection{Blanket}

A fraction (\texttt{1 - exp(}$d$\texttt{/declblkt)}) of the neutron power
incident on the blanket is assumed to be absorbed in the blanket, where input
parameter \texttt{declblkt} is the neutron decay length in the blanket, and
$d$ is the blanket thickness (the calculations are performed separately for
the inboard and outboard regions, as is the case for the first wall and
shield). This absorbed neutron power is multiplied by the energy
multiplication factor \texttt{emult} to calculate its contribution to the
thermal power within the blanket.

If switch \texttt{blktcycle = 0}, the blanket coolant pump power is a fraction
\texttt{fpumpblkt} of the non-pumping thermal power absorbed by the blanket.

If switch \texttt{blktcycle > 0}, the detailed thermal-hydraulic model
described above is used to calculate the blanket coolant pump power.

The efficiency of the blanket coolant pump is again \texttt{etahtp}.

\subsubsection{Shield}
\label{sec:shldpower}

A fraction (\texttt{1 - exp(}$d$\texttt{/declshld)}) of the neutron power
incident on the shield is assumed to be absorbed in the shield, where input
parameter \texttt{declshld} is the neutron decay length in the shield, and
$d$ is the shield thickness.

The shield coolant pump power is a fraction \texttt{fpumpshld} of the
non-pumping thermal power absorbed by the shield, and the efficiency of the
shield coolant pump is \texttt{etahtp}.
...
quantity \texttt{etath}. The power lost via the turbines' inefficiency
escapes from the plant to the environment.

If switch \texttt{blktcycle = 0} (see Section~\ref{sec:blanket_switches}), the
thermal to electric conversion efficiency \texttt{etath} depends on the
blanket type (as given by switch \texttt{blkttype}) and on whether the
divertor heat is used as primary power (via switch \texttt{iprimdiv}; see
Section~\ref{sec:divpower}). Table~\ref{tab:etath} summarises these options.
...
If switch \texttt{blktcycle = 1}, the input value for the thermal to electric
conversion efficiency \texttt{etath} is used.

If switch \texttt{blktcycle = 2}, an equation for \texttt{etath} ($\eta$)
assuming a steam Rankine cycle is used~\cite{Harrington_bop}:
\begin{eqnarray}
\eta & = & -2.265 + 0.932 \, \log(T + 49.999) \;\;\; \mbox{\small(water coolant;
  saturated steam Rankine cycle)} \\
\eta & = & -0.890 + 0.442 \, \log(T + 49.088) \;\;\; \mbox{\small(helium coolant;
  superheated steam Rankine cycle)}
\end{eqnarray}

If switch \texttt{blktcycle = 3}, an equation for \texttt{etath} ($\eta$)
assuming a supercritical CO$_2$ Brayton cycle is used~\cite{Harrington_bop}:
\begin{equation}
\eta = -1.873 + 0.804 \, \log(T + 124.061) \;\;\; \mbox{\small(water or
  helium coolant}
\end{equation}
...
heating power (in MW) to be applied to the plasma. This variable may be
...
materials that are present and in what proportions.%%% \Red{[expand one day]} %%%
\begin{description}
...
\subsection{$N^{th}$ of a kind costs}
...
\subsection{Level of safety assurance}
...
\subsection{Replaceable components}
...
\subsection{Plant availability}

Switch \texttt{iavail} is used to control how the overall plant availability
factor \texttt{cfactr} is calculated. If \texttt{iavail = 0}, the input value
of \texttt{cfactr} is used. If \texttt{iavail = 1}, a plant availability model
by N.~Taylor and D.~Ward~\cite{TaylorWard_avail1999} is used instead, in which
\texttt{cfactr} is calculated taking into account the time taken to replace
certain components of the fusion power core, and various unplanned
unavailability fractions which may be set by the user, as summarised in
Table~\ref{tab:availability}.
\begin{table}[tbph]
\begin{center}
...
If \texttt{iavail = 2}, a new plant availability model based on data from
the EUROfusion projects is implemented. The model is split into several parts. 
\begin{enumerate}
 \item Calculation of the planned unavailability - uses data from \cite{Crofts2014} to determine the length of the 
       planned maintenance period depending on the number of remote handling systems operating in parallel. The number 
       of planned maintenance periods is determined by the estimated lifetimes of the blanket and divertor based on the
       heat and neutron loads.
 \item Calculation of the unplanned unavailability - for this the plant is split into a number of subsystems each with their
       own calculation for the unplanned unavailability. The aim of the model it to try and capture the increased risk to 
       availability when approaching technological limits. The breakdown is:
       \begin{itemize}
        \item Magnets
        \item Divertor
        \item First wall and blanket
        \item Balance of plant
        \item Heating and current drive
        \item Cryogenic systems
       \end{itemize}
       The list is not exhaustive or contains adequate detail for a full RAMI analysis but it allows
       PROCESS to include some idea of the plant parameters than can influence the availability.

 \item Calculation of relevant parameters (e.g. total availability, capacity factor, etc.).
\end{enumerate}



\subsection{Cost of electricity calculations}
...
\subsection{Net electric power calculation}
