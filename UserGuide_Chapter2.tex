...
constraints (see Section~\ref{sec:GNPP}). It is designed to solve the
necessary but not sufficient conditions of a constrained optimum, hence, the
solution does not have to be a global optimum (see Section~\ref{sec:Lagrange}).

The detailed algorithm is explained in Appendix~\ref{app:Opt} and is based on
the Lagrange method (see Section~\ref{sec:Lagrange}) that combines both the
objective function and the constraints using Lagrange multipliers. It applies
a sequential quadratic programming approach (Section~\ref{sec:SQP}) in which a
series of subproblems is solved that constitute a local quadratic expansion of
the Lagrangian (Section~\ref{sec:QSP}). The solution of the quadratic
subproblem is the direction of a line search along which a one dimensional
optimisation is performed (Section~\ref{sec:linesearch}). This line search was
...
estimated using a variant of the Broyden-Fletcher-Goldfarb-Shanno update (see
Section~\ref{sec:BFGS}).
...
\put(50.0,197.0){\makebox(0,0){initialise variables}}
\put(50.0,185.0){\makebox(0,0){input from file}}
\put(50.0,170.0){\makebox(0,0){define free parameters}}
\put(50.0,164.5){\makebox(0,0){(iteration variables)}}
\put(50.0,159.0){\makebox(0,0){and their bounds}}
%\put(50.0,155.0){\makebox(0,0){define constraints}}
%\put(50.0,155.0){\makebox(0,0){to be obeyed}}
\put(50.0,137.0){\makebox(0,0){define constraints}}
...
\put(30.0,194.0){\framebox(40.0,6.0){}}
\put(34.0,182.0){\framebox(32.0,6.0){}}
\put(28.0,156.0){\framebox(44.0,17.0){}}
%\put(34.0,152.0){\framebox(32.0,6.0){}}
\put(30.0,134.0){\framebox(40.0,6.0){}}
...
input (see Figures~\ref{fig:flow_hybrd} and \ref{fig:flow_vmcon}). Tables~\ref{tab:itvars1}, 
\ref{tab:itvars2} and~\ref{tab:itvars3} list the iteration variables available in \process. 
...
28  & \texttt{fradpwr}  & f-value for radiation power limit equation    & 17  & \texttt{0.001D0} & \texttt{0.990D0} \\
...
{\label{tab:itvars1}
  \textit{Iteration variables 1 to 55 present in \process. The f-values correspond to the
    ...
    variables are shown in Tables~\ref{tab:itvars2} and \ref{tab:itvars3}.}
}
\end{table}
\clearpage
\normalsize
...
98  & \texttt{li6enrich}& lithium-6 enrichment percentage (\texttt{blktmodel=1})  &     & \texttt{7.400D0} & \texttt{100.0D0} \\
99  & \texttt{}         & (OBSOLETE)                                              &   & - & - \\
100 & \texttt{}         & (OBSOLETE)                                              &   & - & - \\
...
107 & \texttt{favail}   & f-value for minimum availability limit equation         & 61  & \texttt{0.001D0} & \texttt{1.000D0} \\
108 & \texttt{breeder\_f}& breeder volume ratio: Li4SiO4/(Be12Ti+Li4SiO4)         &     & \texttt{0.060D0} & \texttt{1.000D0} \\
109 & \texttt{ralpne}   & thermal alpha density / electron density                &     & \texttt{0.050D0} & \texttt{0.150D0} \\
110 & \texttt{ftaulimit}& f-value for limit on ratio of $\alpha$ part. to energy conf. times & 62  & \texttt{0.001D0} & \texttt{1.000D0} \\
\hline
\end{tabular}
\end{center}
\caption[List of iteration variables 56 onwards]
{\label{tab:itvars2}
  \textit{Iteration variables 56 onwards. The f-values correspond to the
    given constraint equations (see Table~\ref{tab:eqns}). The other iteration
    variables are shown in Tables~\ref{tab:itvars1} and \ref{tab:itvars3}.}
}
\end{table}
\clearpage
\normalsize
...
\texttt{ixc} &          &                                               & \texttt{icc} & lower        & upper       \\
no. & variable name     & description                                   & eqn & bound        & bound       \\ \hline
111 & \texttt{fniterpump}& f-value for constraint that number of pumps $<$ tfno     & 63  & \texttt{0.001D0} & \texttt{1.000D0} \\
112 & \texttt{fzeffmax}  & f-value for Zeff limit equation                        & 64  & \texttt{0.001D0} & \texttt{1.000D0} \\
113 & \texttt{ftaucq}  & f-value for quench time limit equation                   & 65  & \texttt{0.001D0} & \texttt{1.000D0} \\
114 & \texttt{fw\_channel\_length}  & Length of a single first wall channel         & 39  & \texttt{0.001D0} & \texttt{1.000D3} \\
115 & \texttt{fpoloidalpower}  & f-value for max rate of change of energy in poloidal field & 66  & \texttt{0.001D0} & \texttt{1.000D0} \\
116 & \texttt{fradwall} & f-value for upper limit on radiation wall load          & 67  & \texttt{0.001D0} & \texttt{1.000D0} \\
\hline
\end{tabular}
\end{center}
\caption[List of iteration variables 111 onwards]
{\label{tab:itvars3}
  \textit{Iteration variables 111 onwards. The f-values correspond to the
    given constraint equations (see Table~\ref{tab:eqns}). The other iteration
    variables are shown in Tables~\ref{tab:itvars1} and \ref{tab:itvars2}.}
}
\end{table}
\clearpage
\normalsize
...
17  & radiation fraction upper limit                            & L    & \textbf{28} \\
...
38  & first wall coolant temperature rise upper limit           & L    & \textbf{62} \\
39  & first wall peak temperature upper limit                   & L    & \textbf{63} \\
...
{\label{tab:eqns}
  \textit{Summary of the first 50 constraint equations present in \process. Consistency
    ...
    (non-exhaustive) iteration variable numbers (see Tables~\ref{tab:itvars1},\ref{tab:itvars2} 
    and~\ref{tab:itvars3}) that directly affect the associated constraint
...
56  & $P_{\mbox{\scriptsize separatrix}}/R_{\mbox{\scriptsize major}}$ upper limit & L    & \textbf{97},1,3,102 \\
...
61  & Minimum availability value limit                          & L    & \textbf{107} \\
62  & Lower limit on the ratio of alpha particle to energy confinement times  & L    & \textbf{110} \\
63  & Number of vacuum pumps (vacuum\_model=simple)             & L    & \textbf{111} \\
64  & Limit on Zeff $\le$ Zeffmax                               & L    & \textbf{112} \\
65  & TF dump time greater than or equal to calculated quench time & L    & \textbf{56}, 113 \\
66  & Limit on rate of change of energy in poloidal field       & L    & \textbf{65, 115 } \\
67  & Simple upper limit on radiation wall load                 & L    & \textbf{116}, 102, 4,6 \\
\hline
\end{tabular}
\end{center}
\caption[List of constraint equations 51 onwards]
{\label{tab:eqns2}
  \textit{Summary of constraint equations 51 onwards. Consistency
    equations are marked C, limit equations are marked L\@. Some
    (non-exhaustive) iteration variable numbers (see Tables~\ref{tab:itvars1}, \ref{tab:itvars2}
    and~\ref{tab:itvars3}) that directly affect the associated constraint
    equations are given, the one listed first being the most relevant.}
...
$\pm 16$        & minimise R0 and maximise burn time (linear combination) \\
$\pm 17$        & net electrical power \\
\hline
\end{tabular}
\end{center}
...
$31$ & \texttt{taulimit}   & Taup/taueff lower limit \\
\hline
\end{tabular}
\end{center}
