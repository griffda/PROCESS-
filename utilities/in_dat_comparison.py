...
def output(kb, kbs, kbd, k1n2, k2n1, f1, f2, c_args):
    ...
    print("\nThere are {0} variables in {1} NOT IN {2}:\n".
        format(len(k1n2), c_args.f[0], c_args.f[1]), file=location)
    print('\n'.join('{0} : {1}'.format(*k) for k in enumerate(k1n2)), file=location)

    print("\nThere are {0} variables in {1} NOT IN {2}:\n".
        format(len(k2n1), c_args.f[1], c_args.f[0]), file=location)
    print('\n'.join('{0} : {1}'.format(*k) for k in enumerate(k2n1)), file=location)

    print("\nThere are {0} variables in BOTH {1} AND {2} with the DIFFERENT values:\n".
        format(len(kbd), c_args.f[0], c_args.f[1]), file=location)
    print('\n'.join("{0} : {1} \n\t {2} = {3} \n\t {4} = {5}\n".
        format(k[0], k[1], c_args.f[0], f1.data[k[1]].value, c_args.f[1], f2.data[k[1]].value) 
        for k in enumerate(kbd)), file=location)
        
    print("\nThere are {0} variables in BOTH {1} AND {2} with the SAME values:\n".
        format(len(kbs), c_args.f[0], c_args.f[1]), file=location)
    print('\n'.join("{0} : {1} \n\t {2} = {3} \n\t {4} = {5}\n".
        format(k[0], k[1], c_args.f[0], f1.data[k[1]].value, c_args.f[1], f2.data[k[1]].value) 
        for k in enumerate(kbs)), file=location)
...
def main(cmd_args):
    ...
    keys_both, keys_both_same, keys_both_diff, keys_1n2, keys_2n1 = \
        compare_input_files(file_1, file_2)

    output(keys_both, keys_both_same, keys_both_diff, keys_1n2, keys_2n1, 
        file_1, file_2, cmd_args)
...
if __name__ == "__main__":
    ...
    PARSER.add_argument("-s", "--save", help="Save output to file called input_comp.txt",
                        action="store_true")
