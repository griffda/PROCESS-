...
#Usage
    ...
    PARSER.add_argument("-x", "--xaxis",
                        default='rmajor',
                        help="x-axis, default=rmajor")

    PARSER.add_argument("-y", "--yaxis",
                        default='powfmw',
                        help="y-axis, default=powfmw")

    PARSER.add_argument("mfiles", metavar='f', type=str,
                        default='MFILE.DAT', nargs='*',
    ...
    ylabel(ARGS.yaxis)
    plot(XARR, YARR, 'ks')
    savefig('Comparison_'+ARGS.xaxis.replace('/', '_')+'_'+\
            ARGS.yaxis.replace('/', '_')+'.'+ARGS.end)
