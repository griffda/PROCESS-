...
def param_comparison(cargs, m_file, loc):
    ...
    same = [key for key in params_in_mfile if PARAMS[key] ==
            m_file.data[key].get_scan(-1)]
    diff = set(params_in_mfile).difference(same)

    print("\nParameters", file=loc)

    print(FAIL+''.join('{0}\n'.format(k) for k in params_not_in_mfile) +
          ENDC, file=loc)

    print(FAIL +
          ''.join("{0:<15} \t {1:<5} = {2:<10.3g} \t {3:<5} = {4:.3g}\n".
                  format(k, cargs.r, PARAMS[k], cargs.f,
                         m_file.data[k].get_scan(-1)) for k in diff) +
          ENDC, file=loc)

    print(OK +
          ''.join("{0:<15} \t {1:<5} = {2:<10.3g} \t {3:<5} = {4:.3g}\n".
                  format(k, cargs.r, PARAMS[k], cargs.f,
                         m_file.data[k].get_scan(-1)) for k in same) +
          ENDC, file=loc)
...
def limit_comparison(cargs, m_file, loc):
    ...
    EXCEEDED = [key for key in LIMIT_KEYS
                if LIMITS[key].type == "-" and
                LIMITS[key].limit < m_file.data[LIMITS[key].parameter].
                get_scan(-1)]

    NOT_EXCEEDED = set([k for k in LIMIT_KEYS if LIMITS[k].type == "-"]).\
        difference(EXCEEDED)

    NOT_MET = [key for key in LIMIT_KEYS
               if LIMITS[key].type == "+" and
               LIMITS[key].limit > m_file.data[LIMITS[key].parameter].
               get_scan(-1)]

    MET = set([k for k in LIMIT_KEYS if LIMITS[k].type == "+"]).\
        difference(NOT_MET)

    print("\nLimits - Upper", file=loc)

    print(FAIL +
          ''.join("{0:<10}\t{1:<20}\t{2:<5}={3:<5.3g}\t{4:<5}={5:.3g}\n".
                  format(k, LIMITS[k].parameter, cargs.r, LIMITS[k].limit,
                         cargs.f, m_file.data[LIMITS[k].parameter].
                         get_scan(-1))
                  for k in EXCEEDED) + ENDC, file=loc)
    ...
    print(OK +
          ''.join("{0:<10}\t{1:<20}\t{2:<5}={3:<5.3g}\t {4:<5}={5:.3g}\n".
                  format(k, LIMITS[k].parameter, cargs.r, LIMITS[k].limit,
                         cargs.f, m_file.data[LIMITS[k].parameter].
                         get_scan(-1))
                  for k in NOT_EXCEEDED) + ENDC, file=loc)
    ...
    print(FAIL +
          ''.join("{0:<10}\t{1:<20}\t{2:<5}={3:<5.3g}\t{4:<5}={5:.3g}\n".
                  format(k, LIMITS[k].parameter, cargs.r, LIMITS[k].limit,
                         cargs.f, m_file.data[LIMITS[k].parameter].
                         get_scan(-1))
                  for k in NOT_MET) + ENDC, file=loc)

    print(OK +
          ''.join("{0:<10}\t{1:<20}\t{2:<5}={3:<5.3g}\t{4:<5}={5:.3g}\n".
                  format(k, LIMITS[k].parameter, cargs.r, LIMITS[k].limit,
                         cargs.f, m_file.data[LIMITS[k].parameter].
                         get_scan(-1)) for k in MET) +
          ENDC, file=loc)
...
if __name__ == "__main__":
    ...
    PARSER.add_argument("-r", metavar='REFNAME', type=str,
                        default="", help='specify reference JSON file')
    ...
    if COMMAND_ARGS.r:
        ...
        REF = json.load(open(COMMAND_ARGS.r),
                        object_pairs_hook=collections.OrderedDict)
        PARAMS = REF["PARAMS"]
        LIMITS_NO_TUPLES = REF["LIMITS"]

        LIMITS = {key: collections.namedtuple("GenericDict",
                                              LIMITS_NO_TUPLES[key].
                                              keys())(**LIMITS_NO_TUPLES[key])
