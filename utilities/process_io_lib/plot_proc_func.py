...
def plotdhgap(axis, inpt, outpt, inthk, outthk, toppt, topthk, delta, col):
    ...
    axis.plot(np.concatenate([rs1, rs2[::-1]]),
              np.concatenate([zs1, zs2[::-1]]), color='black')
    axis.plot(np.concatenate([rs3, rs4[::-1]]),
              -np.concatenate([zs3, zs4[::-1]]), color='black')
...
def plot_shield_i_single_null(axis, mfile_data, scan):
    ...
    plotdhgap(axis, c_shldith, c_blnkoth, blnkith, blnkoth, a[11],
              -blnktth, triang, FILLCOLS[3])
    plotdhgap(axis, c_blnkith, c_fwoth, fwith, fwoth,
              a[11]+blnktth, -tfwvt, triang, FILLCOLS[4])
...
def plot_magnetics_info(axis, mfile_data, scan):
    ...
    data = [(pf_info[0][0], pf_info[0][1], "MA"),
            ...
            ("sig_tf_case_max", "Allowable maximum shear stress in the TF case (Tresca criterion)", "Pa"),
            ("sig_tf_wp_max", "Allowable maximum shear stress in the TF conduit (Tresca criterion)", "Pa"),
            ("", "", ""),
            ("#Costs", "", ""),
...
def plot_current_drive_info(axis, mfile_data, scan):
    ...
    data = [(pinjie, "SS auxiliary power", "MW"),
