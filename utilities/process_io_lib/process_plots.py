...
def plot_pulse_timings(mfile_obj, save=False, show=False, return_fig=False, save_path=""):
    """
    ...
    for m in range(n_time):
        ...
        current_arrays[number_of_pf_circuits-2][m] = mfile_obj.data[variable_name].get_scan(-1)/1e6

    # Plasma
    for p in range(n_time):
        variable_name = "plasmat{0:02}".format(p+1)
        current_arrays[number_of_pf_circuits-1][p] = mfile_obj.data[variable_name].get_scan(-1)/1e6

    # Plotting

    # Axis 1
    plot_cs_current = axis_1.plot(time, current_arrays[-2], color=color_sequence[0], label="Central Solenoid")
    ...
    # Axis 2
    plot_plasma = axis_2.plot(time, current_arrays[-1], color=color_sequence[2], label="Plasma")
    ...
    for s in range(number_of_pf_circuits-2):
        axis_3.plot(time, current_arrays[s], label="PF circuit {0}".format(s+1), color=color_sequence[s])
    ...
    for i in range(len(time_list)):
        ...
        if time_length != 0:
            patch_handles.append(axis_0.barh(y_pos, time_length, color=color_sequence[i], 
                align='center', left=left, height=5, label=time_list[i]))
            
            # accumulate the left-hand offsets
            left += time_length

    axis_0.legend(bbox_to_anchor=(0,1.1,1,0.2), loc="lower left",
                mode="expand", borderaxespad=0, ncol=15, labelspacing=2)
    axis_0.axes.get_yaxis().set_visible(False)
    axis_0.tick_params(axis=u'both', which=u'both',length=0)

    xticklabels = axis_0.get_xticklabels() + axis_1.get_xticklabels() + axis_2.get_xticklabels()
    plt.setp(xticklabels, visible=False)

    # Options
    if save:
        plt.savefig('{0}process_times.png'.format(save_path), dpi=300, bbox_inches='tight')
...
def radial_bar_plot(build_params, mfile_obj, save=False, show=False, return_fig=False, save_path=""):
    """
    ...
    for i in range(len(build_params)):
        ...
        if item_thickness != 0:
            patch_handles.append(ax.barh(y_pos, item_thickness, color=color_sequence[i], align='center', left=left, height=5, label=build_params[i]))
    ...
    # Options
    if save:
        plt.savefig('{0}process_radial_plot.png'.format(save_path), dpi=300, bbox_inches='tight')
...
def plasma_profiles_plot(mfile_obj, save=False, show=False, return_fig=False, save_path=""):
    """
    ...
    axis_3.legend()
     ...
     # Options
    if save:
        plt.savefig('{0}process_plasma_profiles.png'.format(save_path), dpi=300, bbox_inches='tight')
