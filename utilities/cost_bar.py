...
def comp_orig():
    ...
    for id, item in enumerate(mfile_list):
        ...
        if "moneyint" not in item.data.keys():
            print('Interest during construction (moneyint) is only calculated for ireactor = 1')
...
def comp_new():
    ...
    'Active maintenance\n and remote handling', 'Vacuum vessel and\n liquid nitrogen plant',
    'CS and PF coils', 'Cryoplant and\n distribution','Electrical power supply\n and distribution',
    'Additional project\n expenditure', 'Other subsystems']
    index = np.arange(len(labels))
    bar_width = 0.7 / len(mfile_list)
    fig, ax = plt.subplots()
    ...
    for id, item in enumerate(mfile_list):
        ...
        cost[5] = item.data["s34"].get_scan(-1)           # Vacuum vessel and liquid nitrogen plant
        cost[6] = item.data["s35"].get_scan(-1)           # System for converting heat to electricity
        cost[7] = item.data["s36"].get_scan(-1)           # CS and PF coils
        cost[8] = item.data["s51"].get_scan(-1)           # Cryoplant and distribution
        cost[9] = item.data["s52"].get_scan(-1)           # Electrical power supply and distribution
        ...
        sizes = [cost[0]+cost[1], cost[2], cost[3], cost[4], cost[5], cost[7],
        cost[8], cost[9], cost[10], cost[6]+cost[11]-cost[7]-cost[8]-cost[9]-cost[10]]
...
if __name__ == '__main__':
    ...
    parser = argparse. \
        ArgumentParser(description="Displays the cost breakdown as a bar chart.  "
