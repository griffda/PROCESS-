...
def ohmic():
    """ Ohmic Power """
    Foh = plascur*plascur/16.0 * R/(a*a*K) * Zeff*(1.0 + 1.5*alphat)/fnc/ Trat**1.5
    Poh = Foh / T0**1.5
...
def brem_process():
    ...
    radexp = (1.0 + alphan)**1.5 * np.sqrt(1.0 + alphan + alphat) / (1.0 + 2.0*alphan + 0.5*alphat)

    deni = dene * ( 1.0 - 2.0*ralpne - (6.0*rncne + 8.0*rnone + 26.0*rnfene) ) # No beam
...
def sync_tnt():
    ...
    fPs = 20.0*fv*4.1E-7*(Trat*B)**2.5 * np.sqrt((1.0 - refl)/10.0/a/np.sqrt(K))
    Psf = fPs*np.sqrt(n0)
...
def sync_process():
    ...
    Ps = 1.3E-4*(B*ten)**2.5 * np.sqrt(1.0+xfact) * np.sqrt(den20/a) * np.sqrt(1.0 - refl) * fv*20.0
...
def transport():
    ...
    if (scaling_law==0):  # IPB98(y,2) scaling
        ...
        ftaue = hfact * 0.0562 * plascur**0.93 * B**0.15 * nbar**0.41 * \
        R**1.39 * a**(0.58) * K**0.78 * afuel**0.19
...
if __name__ == '__main__':
    ...
    parser = argparse. \
        ArgumentParser(description="Displays a POPCON plot for input MFILE.  "
    ...
    parser.add_argument("-t", "--test", help="Test mode: ignores MFILE and runs with R Kembleton's test values",
                        action="store_true")
    ...
    parser.add_argument("-zimp", type=float, default="18.0",
                      help="Impurity charge (default=18 Ar)")
    ...
    s4 = 4.16*((1.0 + 1.5*alphat)*plascur/5.0/a/B/K)**2 / (2.0 + 4.5*alphat)/s3
    ...
    if (fuel2=='T'):
        ...
        fion = Trat + (Zimp*(Zimp-Zeff) + 1.8*Zeff - 2.6) / Zimp / (1.8*Zimp - 2.6)
    ...
    # Plotting
    if (Pplot==0):
        plt.contourf(Pbal,60,extent=[np.min(xplot),np.max(xplot),np.max(yplot),np.min(yplot)],cmap='jet')
        plt.colorbar(label='$P_{aux}$ / MW')
        plt.contour(Pbal,levels=[0.0],extent=[np.min(xplot),np.max(xplot),np.max(yplot),np.min(yplot)],colors='k')
    elif (Pplot==1):
        plt.contourf(Pnet,60,extent=[np.min(xplot),np.max(xplot),np.max(yplot),np.min(yplot)],cmap='jet')
        plt.colorbar(label='$P_{net}$ / MW')
        plt.contour(Pnet,levels=[0.0],extent=[np.min(xplot),np.max(xplot),np.max(yplot),np.min(yplot)],colors='k')
    elif (Pplot==2):
        plt.contourf(5.0*Palf,60,extent=[np.min(xplot),np.max(xplot),np.max(yplot),np.min(yplot)],cmap='jet')
