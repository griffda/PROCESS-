...
def plotProfile(n,i,p,count):
    ...
    for f in range(0,n):
        plt.scatter(DATA_PROFILES[f][0], DATA_PROFILES[f][i])
        plt.plot(DATA_PROFILES[f][0],DATA_PROFILES[f][i], label=LEGEND_NAME[f])
...
def plot_nprofile(prof):  #cut-down version from plot_proc.py
    ...
    ne = ne/1e19
    prof.plot(rho,ne, label="PROCESS 0D")
    prof.legend()
...
def plot_tprofile(prof):
    ...
    prof.plot(rho,te, label="PROCESS 0D")
    prof.legend()
...
def plot_qprofile(prof):
    ...
    prof.plot(rho,q_r_sauter, label="PROCESS 0D")
    prof.legend()
...
#Usage
    ...
    NUM_PAGES = 4                         #number of pages required
    PLOTS_PAGE = 4                        #number of plots per page
