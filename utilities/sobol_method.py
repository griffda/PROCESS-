...
def write_Sobol_Output(X, S):
    ...
    for i in range(X["num_vars"]):
        f.write(
            "%s %f %f %f %f\n"
            % (X["names"][i], S["S1"][i], S["S1_conf"][i], S["ST"][i], S["ST_conf"][i])
        )
...
if __name__ == "__main__":
    ...
    PARSER.add_argument(
        "-i",
        "--inputfile",
        default="sobol_method_conf.json",
        type=str,
    ...
    PARSER.add_argument(
        "-e",
        metavar="ERRORLIST",
        default="output_failed_solutions.txt",
        type=str,
    ...
    PARSER.add_argument(
        "-c",
        metavar="CONVLIST",
        default="output_conv_solutions.txt",
        type=str,
    ...
    # Get parameters
    Get_Input()
    DICTS_FILE_PATH = os.path.join(os.path.dirname(__file__), "sobol_method_conf.json")
    ...
    params_sol = sobol.analyze(
        params_bounds, sols, calc_second_order=False, print_to_console=True
    )
