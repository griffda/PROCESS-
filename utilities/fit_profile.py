...
#Usage
 ...
 default = profile.txt")
    ...
    PARSER.add_argument("-r", "--rho", type=int,
                        default=0,
...
#####################################
 ...
 Remember to start counting at 0!')
    ...
    try:
        ...
        print('Warning: The column for the temperature does not exist!\
 Remember to start counting at 0!')
...
###################################################
    ...
    if USEDENSITY:
        ...
        if ARGS.rhopedn >= 0. and ARGS.rhopedn <= 1.:
        ...
        if LSQFIT[-1] not in [1, 2, 3, 4]:
            ...
            axhline(NPED, ls=':')
            axhline(NSEP)
            plot(RHOARR, [nprofile(x, RHOPEDN, NCORE, NPED, NSEP, ALPHAN)
                       for x in RHOARR])
            plot(RHO, DEN, 'kx')


    if USETEMPERATURE:
        DICTFIT = {'alphat':2., 'betat':2.}

        if ARGS.rhopedt >= 0. and ARGS.rhopedt <= 1.:
        ...
        LSQFIT = leastsq(tresidual,
                         array([DICTFIT['alphat'], DICTFIT['betat'],
                                DICTFIT['rhopedt']]),
        ...
        if LSQFIT[-1] not in [1, 2, 3, 4]:
            ...
            axhline(TPED, ls=':')
            axhline(TSEP)
            plot(RHOARR, [tprofile(x, RHOPEDT, TCORE, TPED, TSEP, ALPHAT, BETAT)
                       for x in RHOARR])
            plot(RHO, TEMP, 'kx')
