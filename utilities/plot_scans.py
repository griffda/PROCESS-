...
if __name__ == '__main__':
    ...
    for ii in range(len(input_files)):
        ...
        if not os.path.isfile(input_files[ii]):
            print('ERROR : The {} MFILE does not exist, skipping it'.format(input_files[ii]))
    ...
    nsweep_dict[17] = "bmaxtf"  # bmxlim the maximum T field upper limit is the scan variable 
    ...
    if not is_2D_scan:
        ...
        for input_file in input_files:
            ...
            for ii in range(n_scan):
                ...
                if ifail == 1:
                    ...
                    print("Warning : Non-convergent scan point : {} = {}".format(scan_var_name, failed_value ))
            ...
            for ii in range(n_scan):
                scan_var_array[input_file][ii] = m_file.data[scan_var_name].get_scan(conv_i[ii])
            ...
            for output_name in output_names:
                ...
                if not output_name in m_file.data.keys():
                    print("Warning : `{}` does not exist in PROCESS dicts".format(output_name))
                ...
                if not output_name in labels :
                    print("Warning : The {} variable LaTeX label is not defined".format(output_name))
                    print("Warning : Please update the 'label' dict")
            ...
            if term_output:
                ...
                print("scan var {} : {}".format(scan_var_name, scan_var_array[input_file]))
                for output_name in output_names:
        ...
        for output_name in output_names:
            ...
            for input_file in input_files:
                ...
                # Plot the graph
                plt.plot( scan_var_array[input_file], output_arrays[input_file][output_name], '--o', label = labl )
        ...
        for ii in range(n_scan_1):
            ...
            for jj in range(n_scan_2):
                ...
                if ifail == 1:
                    ...
                    print("Warning : Non-convergent scan point : ({},{}) = ({},{})".format(scan_var_name, scan_2_var_name, failed_value_1, failed_value_2 ))
                    print("Warning : This point will not be shown.")
        ...
        for output_name in output_names:
            ...
            for conv_j in conv_ij:
                ...
                for jj in range(len(conv_j)):
                    scan_1_var_array[jj] = m_file.data[scan_var_name]  .get_scan(conv_j[jj])
                    scan_2_var_array[jj] = m_file.data[scan_2_var_name].get_scan(conv_j[jj])
            ...
            plt.yticks( size = axis_tick_size )
            plt.tight_layout()
            plt.savefig( '{}/scan_{}_vs_{}_{}.{}'.format(outdir, output_name, scan_var_name, scan_2_var_name, save_format) )
            plt.clf()
            plt.cla()