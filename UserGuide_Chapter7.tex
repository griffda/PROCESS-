...
\textit{Git}\ repository are to be made, and how new code releases are
performed. We assume that readers have a working knowledge of \textit{Git}\
commands.

\subsection{Access to the source code}

To gain access to the \process\ source code \textit{Git} repository, you need
...
\item Once this has worked (!), contact the \process\ ``Master'' (currently
  Peter Knight), who will add you to \texttt{GitLab} as a \process\
  ``Developer''.
...
\item \texttt{git checkout develop}  (just to ensure that this is the branch
  from which you will be branching \textit{from})
