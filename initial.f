C----------------------------------------------------------------------
C--SCCS information
C  Module         : $Id: initial.f,v 3.48 2006/05/25 09:27:02 pknight Exp pknight $
C  Module name    : $RCSfile: initial.f,v $
C  Version no.    : $Revision: 3.48 $
...
C--Version number 3.150
C
C--Description
C  Routine to initialise all the COMMON block variables.
C  N.B. Many of these variables are re-initialised elsewhere in the
C  code, but are set to zero in this routine anyway.
C
C--Author
...
C  
C--History
C  08/10/96 PJK 3.000 Initial upgraded version
C  20/01/97 PJK 3.001 Added htpmw
C  22/01/97 PJK 3.002 Subsumed heattr.h, heatrinp.h and pfelect.h into
C                     htpwr.h
C  05/02/97 PJK 3.003 Added Martensitic steel fractions fms...
C  13/02/97 PJK 3.004 Changed initial value of fwlife to zero (no
C                     longer an input parameter, but calculated)
C  21/03/97 PJK 3.100 Added call to initialise IFE variables
C                     and added new iteration variables 81--86
C  18/11/97 PJK 3.101 Added ITER-97 scaling laws, IVMS,DRTOP,DZTOP
C  01/04/98 PJK 3.102 Added ITER-96P scaling law, IGNITE, DNLA
C  24/04/98 PJK 3.103 Added IMPC,IMPFE,IMPO,FKBLKT
C  17/07/98 PJK 3.104 Added PTHRMW
C  08/10/98 PJK 3.105 Added new ITER H98 scaling laws
C  19/01/99 PJK 3.106 Added IGEOM, POWERHT
C  19/05/99 PJK 3.107 Added new availability model variables
C  06/07/99 PJK 3.108 Added BCRITSC, JCRITSC, TCRITSC
C  25/04/02 PJK 3.109 Added ZEFFDIV
C  15/06/04 PJK 3.110 Added IPRIMHTP
C  22/05/06 PJK 3.120 Added IFALPHAP
...
C *** Quantities used in inequality constraints :
...
C *** PF coil volt second and inductance information :
...
C *** Reversed field pinch parameters :
...
C *** See which type of device is being modelled

      call devtyp

C *** Initialise stellarator parameters if necessary

      if (istell.eq.1) call stinit

C *** Initialise inertial fusion energy parameters if necessary

      if (ife.eq.1) call ifeini
...
C
C--Description
C  Routine to determine which type of device is to be modelled.
C
C--Author
...
C  08/10/96 PJK 1.010 Fixed error: (istell.gt.2) should be (idev.gt.2)
C  14/03/97 PJK 1.020 idev=3 ==> inertial fusion power plant
C
C--Arguments
C  None
C
...
C *** Read a second input file. If the file does not exist, then the
C *** standard tokamak option is assumed.

      INQUIRE(FILE='device.dat',EXIST=iexist)

      if (iexist) then

C ***    File exists, so read in value for IDEV
...
C ***    Check for illegal values
         ...
         if ((idev.lt.0).or.(idev.gt.3)) then
            write(*,*) 'Error in routine DEVTYP:'
            write(*,*) 'Illegal value for IDEV, = ',idev
            write(*,*) 'Please check file device.dat'
            write(*,*) 'PROCESS stopping.'
            STOP
         end if

C ***    Set relevant switch
...
C ***       Reversed Field Pinch model
            irfp = 1

            else if (idev.eq.3) then

C ***       Inertial Fusion Energy  model
            ife = 1
...
C
C--Description
C  Routine to reset specific variables if certain options are
C  being used.
C
C--Author
C  Peter Knight D3/G12 Culham Laboratory, ext.3330
...
C  
C--History
C  08/10/96 PJK 1.000 Initial upgraded version
C  23/01/97 PJK 1.010 Moved resetting of trithtmw from POWER
C  14/03/97 PJK 1.020 Added coding relevant to IFE device
C  01/04/98 PJK 1.030 Added rnbeam reset for no NBI
C  19/01/99 PJK 1.040 Added warning about IITER flag with
C                     non-ITER profiles
C
C--Arguments
C  None
...
C *** Tight aspect ratio options 
      ...
      if (itart.eq.1) then
         ...
         if (ibss.eq.1) then
            write(nout,*)
     +           'ibss = 1 is not a valid option for a TART device.'
...
C *** Reversed Field Pinch model
      ...
      if (irfp.eq.1) then
         if (itart.eq.1) then
            write(nout,*)
     +           'itart = 1 is not a valid option for the RFP model'
            write(nout,*) 'PROCESS stopping.'
            STOP
         end if
         bcylth   = 0.0D0
...
C *** Ensure that if TF coils are non-superconducting,
C *** only simple stress calculations are performed

      if (itfsup.eq.0) itfmod = 0

C *** PF coil resistivity is zero if superconducting

      if (ipfres.eq.0) pfclres = 0.0D0

C *** If there is no NBI, then hot beam density should be zero

      if (irfcd.eq.1) then
         if ((iefrf.ne.5).and.(iefrf.ne.8)) then
            rnbeam = 0.0D0
         end if
      else
         rnbeam = 0.0D0
...
C *** Check on use of iiter

      if ((iiter.ne.0).and.
     +     ((alphan.ne.0.5D0).or.(alphat.ne.1.0D0))) then
