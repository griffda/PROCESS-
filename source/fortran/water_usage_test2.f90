PROGRAM water_use_module
    ...
    ! Variables
    INTEGER sys_inv, typ_wat, wat_src

    ! Inputs
    DOUBLE PRECISION cap_desal, conc_cycle, conc_nh3, &
    desal_rate, discnt_rate, dislv_solid, drift_mwh, dt_ca, dt_cr, dt_pond, dt_ta, &
    elec_level, eta, evap_mwh, evap_pond, evap_riv, flow_riv, km2_pond, km2_pond_mwh, makeup_mwh, &
    mix_riv, mwh_bleed, mwh_cons, p_ratio, p_sat, p_therm, pgrossmw, plant_avail, plant_life, pond_bleed, q_cr, q_cr_m, q_coeff, &
    q_coeff_wat, q_desal, q_lat_desal, r_h, spd_wind, sup_bleed, sup_evap, sup_mkup, sup_recirc, &
    suspnd_solid, t_c, t_c_m, t_desal, t_dry, t_pond, t_steam, t_sw, t_wb, therm, typ_cool, typ_mech, w_fr, &
    ...
    CALL Nat_draft_evap(t_dry, typ_mech, t_sw, dt_ta, dt_cr, dt_ca, r_h, pgrossmw, eta, t_desal, &
    cap_desal, desal_rate, Wat_evap, Wat_evap_rate)
    CALL Nat_draft_drift(t_dry, typ_mech, t_sw, dt_ta, dt_cr, dt_ca, r_h, pgrossmw, eta, t_desal, &
    cap_desal, desal_rate, Wat_drift)
    CALL Nat_draft_bleed(t_dry, typ_mech, t_sw, dt_ta, dt_cr, dt_ca, r_h, pgrossmw, eta, t_desal, &
    cap_desal, desal_rate, conc_cycle, Wat_bleed)
    CALL Nat_draft_makeup(t_dry, typ_mech, t_sw, dt_ta, dt_cr, dt_ca, r_h, pgrossmw, eta, t_desal, &
    cap_desal, desal_rate, conc_cycle, wat_mkup)
    CALL Nat_draft_consump(t_dry, typ_mech, t_sw, dt_ta, dt_cr, dt_ca, r_h, pgrossmw, eta, t_desal, &
    ...
    CALL sys_inv_ch(wat_src, typ_wat, sys_inv)
    ...
    CALL Wat_conv(wat_evap_array(INT(typ_cool)), pgrossmw, Evap_MWh)
    CALL Wat_conv(drift_array(INT(typ_cool)), pgrossmw, Drift_MWh)
    CALL Wat_conv(wat_dis_array(INT(typ_cool)), pgrossmw, mwh_bleed)
    CALL Wat_conv(wat_makeup, pgrossmw, Makeup_MWh)
    CALL Wat_conv(Wat_consump, pgrossmw, mwh_cons)
...
SUBROUTINE sys_inv_ch(wat_src, typ_wat, sys_inv)
    IMPLICIT NONE
...
!              wat_con_mod()
!              Ct_water_evap()
SUBROUTINE Nat_draft_evap(t_dry, typ_cool, t_sw, dt_ta, dt_cr, dt_ca, r_h, pgrossmw, eta, t_desal, &
    cap_desal, desal_rate, Wat_evap, Wat_evap_rate)
    IMPLICIT NONE
    DOUBLE PRECISION, INTENT(INOUT) :: t_dry, typ_cool, t_sw, dt_ta, dt_cr, dt_ca, r_h, pgrossmw, eta
...
!              Heat_con_mod()
!              wat_con_mod()
SUBROUTINE Nat_draft_drift(t_dry, typ_cool, t_sw, dt_ta, dt_cr, dt_ca, r_h, pgrossmw, eta, t_desal, &
    cap_desal, desal_rate, Wat_drift)
    IMPLICIT NONE
    DOUBLE PRECISION, INTENT(INOUT) :: t_dry, typ_cool, t_sw, dt_ta, dt_cr, dt_ca, r_h, pgrossmw, eta
...
!              Nat_draft_drift()
SUBROUTINE Nat_draft_bleed(t_dry, typ_cool, t_sw, dt_ta, dt_cr, dt_ca, r_h, pgrossmw, eta, t_desal, &
    cap_desal, desal_rate, conc_cycle, Wat_bleed)
    IMPLICIT NONE
    DOUBLE PRECISION, INTENT(INOUT) :: t_dry, typ_cool, t_sw, dt_ta, dt_cr, dt_ca, r_h, pgrossmw, eta, &
    ...
    CALL Nat_draft_evap(t_dry, typ_cool, t_sw, dt_ta, dt_cr, dt_ca, r_h, pgrossmw, eta, t_desal, &
    cap_desal, desal_rate, Wat_evap, Wat_evap_rate)
    CALL Nat_draft_drift(t_dry, typ_cool, t_sw, dt_ta, dt_cr, dt_ca, r_h, pgrossmw, eta, t_desal, &
    cap_desal, desal_rate, Wat_drift)
...
!              Nat_draft_drift()
!              Nat_draft_bleed()
SUBROUTINE Nat_draft_makeup(t_dry, typ_cool, t_sw, dt_ta, dt_cr, dt_ca, r_h, pgrossmw, eta, t_desal, &
    cap_desal, desal_rate, conc_cycle, wat_mkup)
    IMPLICIT NONE
    DOUBLE PRECISION, INTENT(INOUT) :: t_dry, typ_cool, t_sw, dt_ta, dt_cr, dt_ca, r_h, pgrossmw, eta, &
    t_desal, cap_desal, desal_rate, conc_cycle
    ...
    CALL Nat_draft_drift(t_dry, typ_cool, t_sw, dt_ta, dt_cr, dt_ca, r_h, pgrossmw, eta, t_desal, &
    cap_desal, desal_rate, Wat_drift)
    CALL Nat_draft_bleed(t_dry, typ_cool, t_sw, dt_ta, dt_cr, dt_ca, r_h, pgrossmw, eta, t_desal, &
    cap_desal, desal_rate, conc_cycle, Wat_bleed)
...
SUBROUTINE Nat_draft_consump(t_dry, typ_cool, t_sw, dt_ta, dt_cr, dt_ca, r_h, pgrossmw, eta, t_desal, &
    cap_desal, desal_rate, conc_cycle, Wat_consump)
    IMPLICIT NONE
    DOUBLE PRECISION, INTENT(INOUT) :: t_dry, typ_cool, t_sw, dt_ta, dt_cr, dt_ca, r_h, pgrossmw, eta, &
    t_desal, cap_desal, desal_rate, conc_cycle
    ...
    CALL Nat_draft_bleed(t_dry, typ_cool, t_sw, dt_ta, dt_cr, dt_ca, r_h, pgrossmw, eta, t_desal, &
    cap_desal, desal_rate, conc_cycle, Wat_bleed)
    CALL Nat_draft_makeup(t_dry, typ_cool, t_sw, dt_ta, dt_cr, dt_ca, r_h, pgrossmw, eta, t_desal, &
    cap_desal, desal_rate, conc_cycle, wat_mkup)
...
! mwh_bleed = Blow-down losses in MWh
! sys_inv -> typ_wat + wat_src_2
