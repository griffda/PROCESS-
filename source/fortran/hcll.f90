module kit_hcll_module
  ...
  ! real(kind=double) :: blnkith = 0.025D0 + 0.375D0 + 0.21D0
  ! real(kind=double) :: blnkoth = 0.025D0 + 0.715D0 + 0.21D0

  ! TODO - vacuum vessel different thicknesses in FF model
  ! real(kind=double) :: ddwi = 0.25D0
  ! real(kind=double) :: ddwo = 0.50D0
  ...
  contains
   ...
   subroutine neutronics
     ...
     call fast_neutron_flux(fwith*100.0D0, dr_bz_ib*100.0D0, dr_mf_ib*100.0D0, &
        shldith*100.0D0, ddwi*100.0D0, frac_vol_steel_bz/100.0D0, frac_vol_pbli_bz/100.0D0, phi_tfc_ib)

     ! Fast neutron flux on the outboard leg (cm-2 s-1)
     call fast_neutron_flux(fwoth*100.0D0, dr_bz_ob*100.0D0, dr_mf_ob*100.0D0, &
        shldoth*100.0D0, ddwi*100.0D0, frac_vol_steel_bz/100.0D0, frac_vol_pbli_bz/100.0D0, phi_tfc_ob)
   ...
   subroutine thermal_hydraulics
     ...
     if (primary_pumping == 0) then
       ...
       htpmw_shld = fpumpshld*0.0D0 ! pnucshld is not available and is very small compared to other powers so set to zero.
   ...
   subroutine display_output
     ...
     call ovarrf(ofile, 'Toroidal length of inboard  blanket module (m)', '(dt_bb_ib)', dt_bb_ib, 'OP ')
     call ovarrf(ofile, 'Poloidal length of inboard  blanket module (m)', '(dp_bb_ib)', dp_bb_ib, 'OP ')
     call ovarrf(ofile, 'Radial length of inboard  blanket module (m)', '(dr_bb_ib)', dr_bb_ib, 'OP ')
     call ovarrf(ofile, 'Toroidal length of outboard  blanket module (m)', '(dt_bb_ob)', dt_bb_ob, 'OP ')
     call ovarrf(ofile, 'Poloidal length of outboard  blanket module (m)', '(dp_bb_ob)', dp_bb_ob, 'OP ')
     call ovarrf(ofile, 'Radial length of outboard  blanket module (m)', '(dr_bb_ob)', dr_bb_ob, 'OP ')
     call ovarrf(ofile, 'Average blanket poloidal perimeter (m)', '', len_act_ib+len_act_ob, 'OP ')

     call ocmmnt(ofile, 'Inboard/outboard total first wall area :')

     call ovarrf(ofile, 'Number of inboard poloidal modules for a blanket sector', '(nb_pol_ib)', nb_pol_ib, 'OP ')
     call ovarrf(ofile, 'Number of inboard toroidal modules for a blanket sector', '(nb_tor_ib)', nb_tor_ib, 'OP ')
     call ovarrf(ofile, 'Number of outboard poloidal modules for a blanket sector', '(nb_pol_ob)', nb_pol_ob, 'OP ')
     call ovarrf(ofile, 'Number of outboard toroidal modules for a blanket sector', '(nb_tor_ob)', nb_tor_ob, 'OP ')
     ...
     call ovarrf(ofile, 'Volume fraction of He in first wall', '(frac_vol_he_fw)', frac_vol_he_fw, 'OP ')
     call ovarrf(ofile, 'Volume fraction of steel in first wall', '(frac_vol_steel_fw)', frac_vol_steel_fw, 'OP ')
     ...
     call ovarrf(ofile, 'Volume fraction of He in breeder zone', '(frac_vol_he_bz)', frac_vol_he_bz)
     call ovarrf(ofile, 'Volume fraction of steel in the breeder zone', '(frac_vol_steel_bz)', frac_vol_steel_bz)
     call ovarrf(ofile, 'Volume fraction of PbLi in the breeder zone', '(frac_vol_pbli_bz)', frac_vol_pbli_bz)
     ...
     call ovarrf(ofile, 'Volume fraction of He in the manifold', '(frac_vol_he_mf)', frac_vol_he_fw)
     call ovarrf(ofile, 'Volume fraction of the steel in the manifold', '(frac_vol_steel_mf)', frac_vol_steel_mf)
     call ovarrf(ofile, 'Volume fraction of PbLi in the manifold', '(frac_vol_pbli_mf)', frac_vol_pbli_mf)

     call ocmmnt(ofile, 'Blanket masses :\n')

     call ovarre(ofile, 'Total mass for an inboard blanket segment (kg)', '(mass_segm_ib)', mass_segm_ib, 'OP ')
     call ovarre(ofile, 'Total mass for an outboard blanket segment (kg)', '(mass_segm_ob)', mass_segm_ob, 'OP ')
     call ovarre(ofile, 'Total mass for a blanket sector (kg)', '(mass_sector)', mass_sector, 'OP ')
     call ovarre(ofile, 'Total He mass (whole blanket) (kg)', '(mass_he_blanket)', mass_he_blanket, 'OP ')
     call ovarre(ofile, 'Total PbLi mass (whole blanket) (kg)', '(mass_pbli_blanket)', mass_pbli_blanket, 'OP ')
     call ovarre(ofile, 'Total steel mass (whole blanket) (kg)', '(mass_steel_blanket)', mass_steel_blanket, 'OP ')
     call ovarre(ofile, 'Total W mass (whole blanket) (kg)', '(mass_w_blanket)', mass_w_blanket, 'OP ')
     call ovarre(ofile, 'Total blanket mass (kg)', '(mass_blanket)', mass_blanket, 'OP ')

     call osubhd(ofile, 'Neutronics :')

     call ovarrf(ofile, 'Radiation heating power into the first wall (MW)', '(pradfw)', pradfw, 'OP ')
     call ovarrf(ofile, 'Nuclear heating power into the first wall (MW)', '(pnucfw)', pnucfw, 'OP ')
     call ovarrf(ofile, 'Nuclear heating power into the blanket (MW)', '(pnucblkt)', pnucblkt, 'OP ')
     call ovarrf(ofile, 'Tritium breeding ration', '(TBR)', TBR, 'OP ')
     call ovarre(ofile, 'Inboard TFC fast neutron flux (cm-2 s-1)', '(phi_tfc_ib)', phi_tfc_ib, 'OP ')
     call ovarre(ofile, 'Outboard TFC fast neutron flux (cm-2 s-1)', '(phi_tfc_ob)', phi_tfc_ob, 'OP ')
