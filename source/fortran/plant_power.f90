...
70  format(' dE/dt (MW)',t22,5(1pe11.3))
  ...
  subroutine power2(outfile,iprint)
    ...
    use physics_variables, only: palpmw, ignite, pinzoneradmw, pradmw, itart, &
...
50     format(t34,'Totals',t50,f8.2,t70,f8.2,t90,f8.2)
    ...
    if (iradloss == 0) then
        ...
        call ovarrf(outfile,'Transport power from scaling law (MW)','(pscalingmw)',pscalingmw, 'OP ')
        call ovarrf(outfile,'Radiation power from inside "coreradius" (MW)','(pinzoneradmw.)',pinzoneradmw, 'OP ')
        call ovarrf(outfile,'Total (MW)','',pscalingmw+pinzoneradmw, 'OP ')
        sum = pscalingmw+pinzoneradmw
