...
null plasma devices). Components are often referred to as being `inboard' or
...
\caption[build] {\label{fig:build}
    ...
    \textit{Schematic diagram of the fusion power core of a typical tokamak
      ...
      components. A double null plasma is assumed (\texttt{snull=0}). Also
      shown are the code variables used to define the thicknesses of the
      components. The arrowed labels adjacent to the axes are the total
      `builds' to that point. The precise locations and sizes of the 
...
plasma with the divertor at the bottom of the machine. The vertical build and
PF coil current scaling algorithms take the value of this switch into account,
although not the plasma geometry at present.
