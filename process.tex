...
57  & TF coil outer leg toroidal thickness lower limit (SCTF)   & L    & \textbf{99},29,13  \\
58  & TF coil outer leg radial thickness limit lower (SCTF)     & L    & \textbf{100},13 \\
\hline
\end{tabular}
\end{center}
...
\texttt{tfcth}; for superconducting coils the outboard thickness is calculated
taking into account input parameter \texttt{casfact}, the cross-sectional area
ratio of the outboard to inboard steel cases.

To ensure that the coil dimensions of the outboard leg are consistent with the
radial and toroidal thicknesses of the winding pack and the other constituents
of the coil shown in the figure, turn on constraint equations no.~57 and~58
with iteration variables no.\ 99 (\texttt{ftftort}) and 100
(\texttt{ftfthko}).
