...
Chapter~\ref{chap:codetools}. Finally, the Appendices give example input files
...
\item The plasma shaping terms (elongation and triangularity) can be
  ...
  whether they should be scaled with the plasma aspect ratio (\texttt{ishape =
    1}). The latter case should only be used with a TART machine. %!%!%! ishape=2
...
The model is largely based on the Helias 5-B stellarator power plant
...
3-D plasma shape typical of stellarators. A VMEC~\cite{vmec} calculation must
have been performed prior to running with \process. The overall size of the
plasma is scaled correctly for the required (mean) major and minor radii for
the machine being modelled~\cite{geiger}.

It is necessary to provide three input files that define the plasma shape:

\begin{enumerate}

\item Set \texttt{vmec\_info\_file} in the input file to the name of a VMEC
  output file with the following format (the numbers given are those for the
  Helias 5-B plasma configuration); in practice only the effective major and
  minor radius values (\texttt{R\_eff} and \texttt{a\_eff}, respectively) and
  the number of field periods are used:
...
{\label{fig:stelldiv}
  \textit{A five-period Helias plasma with island divertor plates shown in red}
}
\end{figure}
...
on in the input file; the code will stop with an error message if this is
attempted.
