...
\begin{equation}
  T_0 = T_{ped} + \frac{1}{3\pi \, \Gamma\left( 1 + \frac{2}{\beta_T} \right) }
  \rho_{ped,T}^{-2}  \left( \langle T \rangle - T_{ped} \, \rho_{ped,T}^2 \right) \gamma
\end{equation}
with 
\begin{equation}
\gamma = \left\{ 
\begin{aligned}
  &\frac{3}{\alpha_T!} \pi \, \Gamma \left( 1+\alpha_T + 2/\beta_T \right)
  &\qquad \textnormal{for integer } \alpha_T \\
  &\left[ (\rho_{ped,T} -1) \left( T_{ped} + 2 \,
      T_{ped}\,\rho_{ped,T} + T_{sep}(2+\rho_{ped}) \right) \right. & \\
  &\left. - 3 \, \Gamma(-\alpha_T) \, \Gamma(1+\alpha_T+2/\beta_T) \right]
  \, \sin(\pi \alpha_T)
...
methods, as summarised here. Note that, at present, these methods all assume
fits to parabolic density and temperature profiles, and do not take into
account the existence of pedestals (\texttt{ipedestal = 1}).
