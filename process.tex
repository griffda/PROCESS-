...
%  mention power threshold scalings
%  
%  
%  
%  
...
15  & burn time (PULSE) 			                         & C    & \Red{obsolete?} \\
...
47  & TF coil toroidal thickness  (RFP)                   & L    & \textbf{76},77,13,3 \\
...
\caption[TABLE_EQNS] {\label{tab:eqns}
  \textit{Summary of the constraint equations present in \process. Consistency equations are
...
43  & \texttt{cfe0}     & iron impurity fraction                        &     & \texttt{1.00D-6} & \texttt{3.00D-3} \\
...
symmetrical (by default). Components are often referred to as being `inboard'
or `outboard', which simply means that they lie at a radius $R$ less than or
greater than $R_0$, respectively, where $R_0$ is the plasma major radius
(\texttt{rmajor}). For the sake of clarity the thicknesses are not drawn to
scale, and the space labelled as the divertor does not indicate in any way the
actual shape of that component.
...
\caption[build] {\label{fig:build}
  \textit{Schematic diagram of the fusion power core of a typical tokamak
    ...
    components. Also shown are the code variables used to define the
    thicknesses of the components. The arrowed labels adjacent to the axes are
    the total `builds' to that point. The precise locations and sizes of the
    PF coils are calculated within the code (see Section~\ref{sec:pfcoils}).}
}
\end{figure}
...
configuration (although this can be changed if desired), with elongation and
triangularity specified by the user. A great number of physics models are
coded within \process\/ to describe the behaviour of the plasma parameters
such as its current, temperature, density, pressure, confinement etc., and
also the various limits that define the stable operating domain.
...
To enforce any of these limits,
% set \texttt{iculdl = 1} and
turn on constraint equation no.~5 with iteration variable no.~9
(\texttt{fdene}).  In addition, switch \texttt{idensl} must be set to the
...
Many energy confinement time scaling laws~\cite{IPDG,172,H90P,iter93h} are
present within \process, for tokamaks, RFPs or stellarators. These are
calculated in routine \texttt{PCOND}. The value of \texttt{isc} determines
which of the scalings is used in the plasma energy balance calculation.
...
\begin{tabular}{||c||l||} \hline
\texttt{isc} & scaling law \\ \hline
\texttt{1}  & Neo-Alcator (ohmic) \\
\texttt{2}  & Mirnov (H-mode) \\
\texttt{3}  & Merezhkin-Muhkovatov (L-mode) \\
\texttt{4}  & Shimomura (H-mode) \\
\texttt{5}  & Kaye-Goldston (L-mode) \\
\texttt{6}  & ITER 89-P (L-mode) \\
\texttt{7}  & ITER 89-O (L-mode) \\
\texttt{8}  & Rebut-Lallia (L-mode) \\
\texttt{9}  & Goldston (L-mode) \\
\texttt{10} & T10 \\
\texttt{11} & JAERI-88 \\
\texttt{12} & Kaye-Big Complex \\
\texttt{13} & ITER H90-P (H-mode) \\
\texttt{14} & ITER Mix (minimum of \texttt{6} and \texttt{7}) \\
\texttt{15} & Riedel (L-mode) \\
\texttt{16} & Christiansen et al. (L-mode) \\
\texttt{17} & Lackner-Gottardi (L-mode) \\
\texttt{18} & Neo-Kaye (L-mode) \\
\texttt{19} & Riedel (H-mode) \\
\texttt{20} & ITER H90-P (amended) \\
\texttt{21} & Large Helical Device (stellarator) \\
\texttt{22} & Gyro-reduced Bohm (stellarator) \\
\texttt{23} & Lackner-Gottardi (stellarator) \\
\texttt{24} & ITER-93H (H-mode) \\
\texttt{25} & TITAN (RFP) \\
\texttt{26} & ITER H-97P ELM-free (H-mode) \\
\texttt{27} & ITER H-97P ELMy (H-mode) \\
\texttt{28} & ITER-96P (L-mode) \\
...
\caption[TABLE_TAU] {\label{tab:scaling_laws}
  \textit{Summary of the energy confinement time scaling laws in
    \process. These laws are all cited in ~\cite{172,IPDG} and references
    therein, with the exception of laws~20~\cite{H90P}, 21~\cite{LHD},
    22~\cite{GRB}, 23~\cite{LG}, 24~\cite{iter93h}, 25~\cite{titan1}, and
    26--36 (various).}
}
\end{table}
...
\item Numerically fitted scaling~\cite{172}: To use a numerically fitted
...
input values \texttt{scrapli} and {\tt scraplo} (\texttt{iscrp = 1}).
...
Switch \texttt{idivrt} controls the overall plasma configuration. At present,
it is only possible to use \texttt{idivrt = 2}, which corresponds to an
up-down symmetric, double null configuration, as the PF coil current scaling
algorithms only allow for this model.
...
\begin{figure}[tbph]
\epsfig{file=cicc.eps,width=160mm}
\caption[CICC] {\label{fig:CICC}
  \textit{Schematic diagram of the cross-section of a superconducting TF coil,
    showing the CICC (Conductor In Cable Conduit) construction. The cable
    space contains superconducting filaments and circulating liquid helium
    coolant.}
}
\end{figure}
...
\tau_E\; (\mbox{Gyro-reduced Bohm~\cite{GRB}: \texttt{isc=22}})
 & = & 0.25 \, B_0^{0.8} \, n^{0.6} \, P^{-0.6} \, a_p^{2.4} \, R_0^{0.6} \\
\tau_E\; (\mbox{Lackner-Gottardi~\cite{LG}: \texttt{isc=23}})
...
used as an iteration variable (no.\ 11).
...
\begin{figure}[tbph]
\epsfig{file=stellarator_windingpack.eps,width=160mm}
\caption[tosh] {\label{fig:stell1}
  \textit{TF coil inboard leg cross-sections for tokamak and stellarator
    devices.}
}
\end{figure}
...
\item There should be no leading spaces before the variable name.
...
$\:$\texttt{* This is a comment with the asterisk not in the first column} \\
...
Switch \texttt{ioptimz} must be set to \texttt{1} for optimisation mode. As
before, the user must decide which constraint equations and iteration
...
outcome. An unfeasible result, however, occurs if \process\/ cannot find a set
of values for the iteration variables which satisfies all the given
...
latter can be checked simply by changing the initial values of the \textit{
  active}\/ iteration variables in the input file, but the former requires
some extra work. This situation arises if there are insufficient iteration
variables for the given constraint equations. It is important to choose the
right number of \textit{useful}\/ iteration variables for the problem to be
...
\item Add the new figure of merit equation to routine \texttt{FUNFOM} in
  ...
  via \texttt{use} statements present at the start of this routine.
...
\textit{``Global Energy Confinement H-Mode Database for ITER''},
\textit{Nuclear Fusion} \textbf{32} (1992) 291-338, Eqn.\ 6
...
\bibitem{Uckan}
N.\ A.\ Uckan et al.,
\textit{Fusion Technology} \textbf{13} (1988) 411

\bibitem{Nuhrenberg}
%%MHD-Theoretical Aspects of Stellarators
J.\ N\"{u}hrenberg et al., \textit{Plasma Physics and Controlled Fusion}, {\bf
35} (1993) B115
...
\bibitem{GRB}
R.\ J.\ Goldston, H.\ Biglari, G.\ W.\ Hammett et al., \textit{Bull.\ Am.\ Phys.\
Soc.}, \textbf{34} (1989) 1964

\bibitem{LG}
K.\ Lackner and N.\ A.\ O.\ Gottardi, \textit{Nuclear Fusion}, \textbf{30} (1990)
767

\bibitem{iter93h}
S.\ Kaye and the ITER Joint Central Team and Home Teams, in \textit{Plasma
