...
61  & \texttt{gapds}    & gap between vacuum vessel and inboard TF coil           &     & \texttt{0.000D0} & \texttt{10.00D0} \\
...
\caption[TABLE_VARS2] {\label{tab:itvars2}
  \textit{Iteration variables 51 to 96 present in \process. The f-values correspond to the
    given constraint equations (see Table~\ref{tab:eqns}). The other iteration
...
The model is based on the Helium-Cooled Pebble Beds (HCPB) DEMO blanket concept
...
\item Breeding Unit (BU), with radial thicknesses (inboard and outboard,
  respectively) \texttt{blbuith}, \texttt{blbuoth}. This consists of beryllium
  (with fraction by volume \texttt{fblbe}), breeder material
  (\texttt{fblbreed}), steel (\texttt{fblss}) and helium coolant. Three forms
  of breeder material are available: lithium orthosilicate (Li$_4$SiO$_4$)
  (chosen by setting \texttt{breedmat = 1}), lithium metatitanate
  (Li$_2$TiO$_3$) (\texttt{breedmat = 2}) or lithium zirconate (Li$_2$ZrO$_3$)
...
Together, the BU, BM and BP make up the `blanket', with total radial
thicknesses \texttt{blnkith} (inboard) and \texttt{blnkoth} (outboard), and
void (coolant) fraction \texttt{vfblkt}; these
...
\item The tritium breeding ratio, \texttt{tbr}. This can be constrained to be
  ...
  (\texttt{ftbr}). The inboard and outboard blanket BU thicknesses,
...
\texttt{gapsto} in Figures~\ref{fig:build_d} and~\ref{fig:build_e}).  Ports
are often necessary for auxiliary power systems etc., and the gaps between
adjacent TF coils can be made large enough to accommodate such equipment,
using constraint equation no.\ 20 with iteration variable no.\ 33
(\texttt{fportsz}).
