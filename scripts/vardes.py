...
MARKDOWN_FILE_PATH = (
    Path(__file__).resolve().parent / "../documentation/proc-pages/vardes.md"
)
...
class VarDes(object):
    ...
    def create_table_heading(self):
        ...
        for i, heading in enumerate(COL_HEADINGS):
            ...
            col_number = str(i + 1)
            headings_str += (
                '\t\t<th class="col' + col_number + '">' + heading + "</th>\n"
            )
...
if __name__ == "__main__":
    ...
    with open(build_directory / "ford_project.pickle", "rb") as ford_pickle:
        ford_project = pickle.load(ford_pickle)
        create_vardes(ford_project)
