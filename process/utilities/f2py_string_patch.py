...
def string_to_f2py_compatible(target:np.ndarray, string:str=None, except_length:bool=False) -> str:
    ...
    if target_type != 'S':
        raise TypeError(f'{target} is not of type string ("|S<>") is instead {str(target.dtype)}')

    if string:
        if len(string) > target_size and except_length:
            raise RuntimeError(f'String string of length {len(string)} is trying to initiate as {target} with length {target_size}')
        elif len(string) > target_size:
            warnings.warn(RuntimeWarning, f'String string of length {len(string)} is trying to initiate as {target} with length \
