\documentclass[hidelinks]{article}
    ...
    helium coolant. The radial position of the maximum magnetic filed (rbmax), is defined
    as the inboard leg plasma facing edge of the current carrying layer: 
    ...
    magnetic field dependency of toroidal magnets, the maximum magnetic filed is given by
    ...
    TF coils (\emph{ritfc}) is deduced 
    ...
    \noi using this peak field PROCESS and the Winding pack radial geometry at mid-plane 
    can calculate the total vertical forces $F_v$ (corresponding to the variable vforce in the code) 
    on the TF coil.
    ...
    with $B_\text{max}$, $R_{B_\mathrm{max}}$, $I_\mathrm{tot}$, $N_\mathrm{TF}$, 
    $\Delta R_\mathrm{WP}$, $R_\mathrm{in}$ and $R_\mathrm{out}$ being the maximum B 
    field (bmaxtf), the radius of the maximum B field (rbmax), the total TF current (ritfc),
    the number of TF coils (n\_tf), the winding pack thickness (thkwp), the winding pack 
    inboard (r\_wp\_outer) and outplane (r\_tf\_outboard\_in + tinstf) mid-plane 
    radius radius at the plasma facing side. At the limit condition 
    $\Delta R_\mathrm{WP} \rightarrow 0$ the vertical force for infinitely thin coils 
    ...
    is given on the 2019 PROCESS eurofusion report. Please note that the vertical stress
    at mid-plane is half of this force for perfect Princeton D shape.
