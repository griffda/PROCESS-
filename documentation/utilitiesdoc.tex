...
\begin{description}
	...
	\item{\textbf{Output:}}
	Displays plot of the cost breakdown to screen.  For the 1990 cost model, the breakdown for direct, indirect and contingency is also shown.  These can be saved with -s argument (cost\_pie.pdf and direct\_cost\_pie.pdf).
...
\begin{description}
	...
	\item{\textbf{Output:}}
	Displays plot of the cost breakdown to screen.  For the 1990 cost model, the breakdown for direct, indirect and contingency is also shown.  These can be saved with -s argument (cost\_bar.pdf and direct\_cost\_bar.pdf).
...
\subsubsection{POPCON plot (popcon.py)}
...
\begin{description}
	...
	\item{\textbf{Output:}}
	Displays POPCON plot to screen.  This can be saved with -s argument (popcon.pdf).
...
\subsubsection{Profile plots (plot\_profiles.py)}

This utility allows for plotting of the temperature and density profiles 
of a number of MFILEs. The options are described below.
...
\subsubsection{VMCON optimization plots (plot\_opti\_process.py)}

Macro plotting a set of information about the \texttt{VMCON} optimization from an output file called \texttt{OPT.DAT}.\\
The file contains 
\begin{itemize}
	\item The \process indexes of the constraints and the variables used for the considered run.
	\item The variables described in table~\ref{tab:output_ploting:opt_plot_var} stored for each \texttt{VMCON} iteration. Please note that only one set of number is associated in per \texttt{VMCON} iteration.
\end{itemize} 
...
\begin{table}[tbph]
	...
	\begin{center}
		\begin{tabular}{l|l|l} 
			\hline
			\hline
			Variable description                     & \process code name & \texttt{VMCON} doc def \\ 
			\hline			
			Normalized figure of merit               & \texttt{abs(obj)}  & $f(x)$\\			
			VMCON convergence criteria               & \texttt{sum}       & $\left| \nabla_x f(\vec{x}^{j-1})^T \cdot \vec{\delta}^{j} \right| +
			\sum^m_{i=1}\left| \lambda^j_i c_i(\vec{x}^{j-1}) \right|$\\			
			Constraints residual quadratic sums      & \texttt{sqsumsq}   & $\sqrt{\sum^{m}_{i=1} c^{2}_i(\vec{x}^{j-1})}$ \\			
			Individual residual values               & \texttt{conf(i)}   & $c_i(\vec{x}^{j-1})$\\			
			Normalized optimization variables values & \texttt{x(i)}      & $\vec{x}^{j-1}$\\
			\hline
			\hline
		\end{tabular}
	\end{center}
	\caption{\label{tab:output_ploting:opt_plot_var} \textit{Variables stored in \texttt{OPT.DAT}.}}
\end{table}

The python plot routines proposes the following plots
\begin{enumerate}
	\item Figure of merit plot :\\ 
	evolution of the figure of merit with the \texttt{VMCON} index
	\item Convergence plot : \\
	\texttt{VMCON} index evolution of 
	\begin{itemize}
		\item The \texttt{VMCON} convergence parameter 
		\item The quadratic sum of the constraints residuals
		\item The maximum between the VMCON convergence parameter and the constraints residual quadratic sum, actually used to test the \process convergence.
	\end{itemize}
	\item Dominant constraints plots : \\
	The last \texttt{VMCON} iteration is used order to rank the constraints with their residual values. This allows to plot the \texttt{VMCON} index evolution of
	\begin{itemize}
		\item The $N_{const}^{dom}$ dominant constraints values ($N_{const}^{dom}$ can be used defined)
		\item The quadratic sum of the dominant constraints
		\item The total quadratic sum of the constraints
	\end{itemize} 
	...
	\item Selected constraint plot : \\
	Any constraints residual evolution can be plotted given its \process ID number. The associated plot will contains
	\begin{itemize}
		\item the selected constraint evolution
		\item The total quadratic sum of the constraints
	\end{itemize}
	This allows a clearer visualization of a given constraint evolution.
	\item Major variable evolution : \\
	The variation amplitude of the optimization varaibles $\max(\vec{x}^{j-1}) - \min(\vec{x}^{j-1})$ is used to rank the variables. This allows to plot the \texttt{VMCON}.
	index evolution of the $N_{const}^{var}$ dominant variables ($N_{const}^{dom}$ can be used defined).
	\item Selected vaiable pair trajectory plot : \\
	Any pair of variables can be selected using their \process ID defined in vardes, to plot the evolution of their trajectory. The color of each points corresponds to the value of the \process convergence criteria (on base 10 logarithm scale). 
\end{enumerate}
...
\noindent The output can be defined in any visual data format supported by pyplot, 
the label font size can set. Please note that to select individual constraints 
or variable pair plots, it is enough to precise the \texttt{-ic} and the 
\texttt{-ixv/-iyv} pair, respectively. 

\noindent If any modification of improvements
are foreseen, please let us know.


\subsubsection{Radial stress distribution plots (plot\_stress\_tf.py)}

\noindent This utility allow to plot the inboard TF radial stress distribution at mid-plane 
and the associated radial displacement. This utility is meat to be used from the utility and
will use the \texttt{SIG\_TF.DAT} file generated in the \texttt{bin/} folder. The user has the
option to plot only the stress plots or the r displacement if he wish. The options are 
the following ones :

\begin{quote}
  \begin{verbatim}
    usage: plot_stress_tf.py [-h] [-p [PLOT_SELEC]] [-sf [SAVE_FORMAT]]
                         [-as [AXIS_FONT_SIZE]]

    Plot TF stress informations 
    ...
    optional arguments:
      ...
      -as [AXIS_FONT_SIZE], --axis_font_size [AXIS_FONT_SIZE]
                            Axis label font size selection (default=18)
  \end{verbatim}
\end{quote}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
