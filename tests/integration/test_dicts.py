    ...
    for old_key, old_value in ref_dicts["DICT_DEFAULT"].items():
        ...
        if old_value is None:
            ...
            try:
                ...
                logger.error(
                    f"{old_key} in new_dicts should be 0.0, None, "
...
def test_module(ref_dicts, new_dicts):
    ...
    MOD_EXCLUSIONS = ["autodoc_data", "calltree_data", "divertor_kallenbach_variables"]
    # Module variables to ignore (defined differently: env vars or functions)
    VAR_EXCLUSIONS = ["ROOTDIR", "INSTALLDIR", "impdir"]
