...
\begin{description}
\item{\textbf{Input:}}
 \texttt{ndscan.conf}, \indat\, \texttt{run\_process.conf} (optional, will be incorporated in ndscan.conf in the future!)
                                
\item{\textbf{Output:}} All MFILES in subdirectory \texttt{DATA}, packaged NetCDF output file as named in configuration file
\end{description}
...
\end{verbatim}
\end{quote}
For the visualisation tool the corresponding NetCDF input file can also be specified with \texttt{-f}. Per default ``Demonstrationdata.nc'' is used.

\paragraph{Configuration Options:}

The configuration file \texttt{ndscan.conf} uses the JSON format\footnote{www.json.org} and has the following style
\begin{framed}
\begin{verbatim}
{
    "Author": "Me",
    ...
    "Title": "Demonstrationdata",
    "RanBefore": false,
    "Optionals": {
        "MakeFailPlot": true,
        "RemoveScanVarsFromIteration": false,
...
\end{verbatim}
\end{framed}
Required input parameters in the configuration file are the Scan Axes, which has to be at least one and can be N-dimensional. For each axis a \texttt{varname}, a \texttt{Lowerbound}, an \texttt{Upperbound} and the number of \texttt{Steps} $>1$ have to be specified. All other parameters in the configuration file are optional.
\begin{description}
\item[\texttt{\_Comment}] Anything in the comment section is for the user only and will be ignored by the program.
\item[\texttt{Description}] The description will be copied into the NetCDF file.
\item[\texttt{Title}] Name of the output NetCDF file (default \texttt{Demonstrationdata}
\item[\texttt{RanBefore}] Flag to indicate whether this scan has already been run.
...
This creates a set of pedestalised profile parameters that provides the best fit to a temperature or density profile given in the input file.  It uses a least squares method and fits the position of the pedestal as well as the peaking factors.  

\begin{description}
\item{\textbf{Input:}}
 \texttt{profile.txt}  This can contain either a density or temperature profile, or both.  It should contain data in columns, as described below.  Comment lines start with \#.
                                
\item{\textbf{Output:}} The profile parameters are output to the terminal.
\end{description}
