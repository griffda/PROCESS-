...
To add a reference case do the following:

\begin{enumerate}
  \item Have an IN.DAT that works with the current version of PROCESS
  \item \textbf{Run PROCESS} to create an OUT.DAT and MFILE.DAT
  \item Create a folder in the location \textbf{/process/test\_suite/test\_files/[test\_name]} with the name of the test case
  \item Rename your IN.DAT, OUT.DAT and MFILE.DAT to \textbf{ref.IN.DAT}, \textbf{ref.MFILE.DAT} and \textbf{ref.OUT.DAT}
  \item Copy reference files to the folder you created
  \item Run test suite to check it works
  \item Push to GitLab (see instructions in PROCESS manual)
\end{enumerate}
